...
int get_active_chain() { return active_chain; }

void set_active_midi_chan(int flag) { active_midi_chan = flag; }

int get_active_midi_chan() { return active_midi_chan; }
...
int get_tuning_pitchbend() { return tuning_pitchbend; }
...
int get_midi_master_chan() { return midi_master_chan; }

// Enable/Disable System messages globally
void set_midi_system_events(int flag) { midi_system_events = flag; }

int get_midi_system_events() { return midi_system_events; }

// MIDI Learning Mode
void set_midi_learning_mode(int mlm) { midi_learning_mode = mlm; }

int get_midi_learning_mode() { return midi_learning_mode; }
...
void set_midi_filter_cc_ignore(uint8_t chan_from, uint8_t cc_from) { set_midi_filter_event_ignore(CTRL_CHANGE, chan_from, cc_from); }
...
void del_midi_filter_cc_map(uint8_t chan_from, uint8_t cc_from) { del_midi_filter_event_map(CTRL_CHANGE, chan_from, cc_from); }
...
int zmip_get_num_devs() { return NUM_ZMIP_DEVS; }

int zmip_get_seq_index() { return ZMIP_SEQ; }

int zmip_get_step_index() { return ZMIP_STEP; }

int zmip_get_int_index() { return ZMIP_FAKE_INT; }

int zmip_get_ctrl_index() { return ZMIP_CTRL; }
...
int zmip_set_route_chains(int iz, int route) {
    ...
    for (int i = 0; i < ZMOP_CTRL; i++) {
        if (!zmop_set_route_from(i, iz, route))
            return 0;
    }
    return 1;
}
...
int zmop_get_num_chains() { return NUM_ZMOP_CHAINS; }

int zmop_get_num_devs() { return NUM_ZMOP_DEVS; }
...
int8_t get_global_transpose() { return global_transpose; }
...
int init_jack_midi(char* name) {
    ...
    for (i = 0; i < NUM_ZMIP_DEVS; i++) {
        ...
        if (!zmip_init(ZMIP_DEV0 + i, port_name, ZMIP_DEV_FLAGS))
            return 0;
    }
    if (!zmip_init(ZMIP_SEQ, "seq_in", ZMIP_SEQ_FLAGS))
        return 0;
    if (!zmip_init(ZMIP_STEP, "step_in", ZMIP_STEP_FLAGS))
        return 0;
    if (!zmip_init(ZMIP_CTRL, "ctrl_in", ZMIP_CTRL_FLAGS))
        return 0;
    if (!zmip_init(ZMIP_FAKE_INT, NULL, ZMIP_INT_FLAGS))
        return 0;
    if (!zmip_init(ZMIP_FAKE_UI, NULL, ZMIP_UI_FLAGS))
        return 0;

    // Init MIDI Output Ports (ZMOPs)
    for (i = ZMOP_CH0; i <= ZMOP_CH15; i++) {
        sprintf(port_name, "ch%d_out", i);
        if (!zmop_init(ZMOP_CH0 + i, port_name, ZMOP_CHAIN_FLAGS))
            return 0;
        // zmop_set_midi_chan(ZMOP_CH0 + i, i);
    }
    if (!zmop_init(ZMOP_MOD, "mod_out", ZMOP_CHAIN_FLAGS))
        return 0;
    zmop_set_midi_chan_all(ZMOP_MOD);
    if (!zmop_init(ZMOP_STEP, "step_out", FLAG_ZMOP_DROPSYSEX | FLAG_ZMOP_DIRECTOUT))
        return 0;
    zmop_set_midi_chan_all(ZMOP_STEP);
    if (!zmop_init(ZMOP_CTRL, "ctrl_out", FLAG_ZMOP_DIRECTOUT))
        return 0;
    zmop_set_midi_chan_all(ZMOP_CTRL);
    for (i = 0; i < NUM_ZMOP_DEVS; i++) {
        sprintf(port_name, "dev%d_out", i);
        if (!zmop_init(ZMOP_DEV0 + i, port_name, FLAG_ZMOP_DIRECTOUT))
            return 0;
    ...
    for (i = 0; i < ZMOP_CTRL; i++) {
        ...
        for (j = 0; j < NUM_ZMIP_DEVS; j++) {
            if (!zmop_set_route_from(i, ZMIP_DEV0 + j, 1))
                return 0;
        }
        // Internal MIDI to all (by default)
        if (!zmop_set_route_from(i, ZMIP_FAKE_INT, 1))
            return 0;
        // MIDI from UI to Chain's ZMOPS
        if (i >= ZMOP_CH0 && i <= ZMOP_MOD) {
            if (!zmop_set_route_from(i, ZMIP_FAKE_UI, 1))
                return 0;
        }
        // MIDI player to all
        if (!zmop_set_route_from(i, ZMIP_SEQ, 1))
            return 0;
        // Step Sequencer playback (ZMIP_STEP) to all ZMOPS except stepseq capture (ZMOP_STEP)
        if (i != ZMOP_STEP) {
            if (!zmop_set_route_from(i, ZMIP_STEP, 1))
                return 0;
        }
    }
                            ...
                            if (event_type == NOTE_OFF || (event_type == NOTE_ON && event_val == 0)) {
                                ...
                                if (zmop->note_state[event_num] == 0) {
                                    for (j = 1; j < NUM_ZMOP_CHAINS; j++) {
                                        ...
                                        if (zmops[xiz].note_state[event_num] > 0 && zmops[xiz].midi_chan >= 0 && zmops[xiz].n_connections > 0 &&
                                            zmops[xiz].route_from_zmips[izmip]) {
                                            zmop = zmops + xiz;
                                            break;
                                        }
                                    }
...
int ui_send_midi_event(uint8_t* event_buffer, int event_size) { zmip_send_midi_event(ZMIP_FAKE_UI, event_buffer, event_size); }

int ui_send_note_off(uint8_t chan, uint8_t note, uint8_t vel) { zmip_send_note_off(ZMIP_FAKE_UI, chan, note, vel); }

int ui_send_note_on(uint8_t chan, uint8_t note, uint8_t vel) { zmip_send_note_on(ZMIP_FAKE_UI, chan, note, vel); }

int ui_send_ccontrol_change(uint8_t chan, uint8_t ctrl, uint8_t val) { zmip_send_ccontrol_change(ZMIP_FAKE_UI, chan, ctrl, val); }

int ui_send_master_ccontrol_change(uint8_t ctrl, uint8_t val) { zmip_send_master_ccontrol_change(ZMIP_FAKE_UI, ctrl, val); }

int ui_send_program_change(uint8_t chan, uint8_t prgm) { zmip_send_program_change(ZMIP_FAKE_UI, chan, prgm); }

int ui_send_chan_press(uint8_t chan, uint8_t val) { zmip_send_chan_press(ZMIP_FAKE_UI, chan, val); }

int ui_send_pitchbend_change(uint8_t chan, uint16_t pb) { zmip_send_pitchbend_change(ZMIP_FAKE_UI, chan, pb); }

int ui_send_all_notes_off() { zmip_send_all_notes_off(ZMIP_FAKE_UI); }

int ui_send_all_notes_off_chain(uint8_t izmop) { zmip_send_all_notes_off_chain(ZMIP_FAKE_UI, izmop); }
...
int ctrlfb_send_midi_event(uint8_t* event_buffer, int event_size) { zmop_send_midi_event(ZMOP_CTRL, event_buffer, event_size); }

int ctrlfb_send_note_off(uint8_t chan, uint8_t note, uint8_t vel) { zmop_send_note_off(ZMOP_CTRL, chan, note, vel); }

int ctrlfb_send_note_on(uint8_t chan, uint8_t note, uint8_t vel) { zmop_send_note_on(ZMOP_CTRL, chan, note, vel); }

int ctrlfb_send_ccontrol_change(uint8_t chan, uint8_t ctrl, uint8_t val) { zmop_send_ccontrol_change(ZMOP_CTRL, chan, ctrl, val); }

int ctrlfb_send_program_change(uint8_t chan, uint8_t prgm) { zmop_send_program_change(ZMOP_CTRL, chan, prgm); }

//-----------------------------------------------------------------------------
// ZMOP_DEV Direct Send Functions
//-----------------------------------------------------------------------------

int dev_send_midi_event(uint8_t idev, uint8_t* event_buffer, int event_size) { zmop_send_midi_event(ZMOP_DEV0 + idev, event_buffer, event_size); }

int dev_send_note_off(uint8_t idev, uint8_t chan, uint8_t note, uint8_t vel) { zmop_send_note_off(ZMOP_DEV0 + idev, chan, note, vel); }

int dev_send_note_on(uint8_t idev, uint8_t chan, uint8_t note, uint8_t vel) { zmop_send_note_on(ZMOP_DEV0 + idev, chan, note, vel); }

int dev_send_ccontrol_change(uint8_t idev, uint8_t chan, uint8_t ctrl, uint8_t val) { zmop_send_ccontrol_change(ZMOP_DEV0 + idev, chan, ctrl, val); }

int dev_send_program_change(uint8_t idev, uint8_t chan, uint8_t prgm) { zmop_send_program_change(ZMOP_DEV0 + idev, chan, prgm); }
...
int get_zynmidi_num_max() { return ZYNMIDI_BUFFER_SIZE >> 2; }

int get_zynmidi_num_pending() { return jack_ringbuffer_read_space(zynmidi_buffer) >> 2; }
