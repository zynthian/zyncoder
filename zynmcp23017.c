...
int pin2index_zynmcp23017(uint16_t pin)
{
	...
	for (i = 0; i < MAX_NUM_MCP23017; i++)
	{
		...
		{
			if (pin >= zynmcp23017s[i].base_pin && pin < (zynmcp23017s[i].base_pin + 16))
				return i;
		}
	}
	return -1;
...
void zynswitch_update_zynmcp23017(uint8_t i)
{
	...
	if (res >= 0)
		update_zynswitch(i, (uint8_t)res);
}
...
void zyncoder_update_zynmcp23017(uint8_t i)
{
	...
	if (res_a >= 0 && res_b >= 0)
		update_zyncoder(i, (uint8_t)res_a, (uint8_t)res_b);
}
