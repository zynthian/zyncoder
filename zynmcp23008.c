...
int8_t zynmcp23008_pin2index(uint16_t pin) {
    ...
    for (i = 0; i < MAX_NUM_MCP23008; i++) {
        if (zynmcp23008s[i].enabled) {
            if (pin >= zynmcp23008s[i].base_pin && pin < (zynmcp23008s[i].base_pin + 8))
                return i;
        }
    }
    return -1;
 ...
 */
uint8_t zynmcp23008_read_pins(uint8_t i) { return wiringPiI2CReadReg8(zynmcp23008s[i].fd, MCP23x08_GPIO); }
...
void end_poll_zynswitches() { end_poll_zynswitches_flag = 1; }
